...
function gettestData () {
  ...
  HLTV.getMatchesStats({
    ...
    .then(res => {
      ...
      fs.writeFile('./test/getMatchesStats.txt', to_write, (err) => console.log(err))
    ...
    .then(MatchMapStats => {
      ...
      fs.writeFile('./test/getMatchMapStats.txt', to_write, (err) => console.log(err))

      sleep(1500)
      var matchPageID = MatchMapStats.matchPageID
      // Keep the round summary data in MatchMapStats to save to DB later
      return Promise.all([HLTV.getMatch({ id: 2335818 }), MatchMapStats])
    })
    ...
    .then(([res, MatchMapStats]) => {
      ...
      fs.writeFile('./test/getMatch.txt', to_write, err => console.log(err))
    })
}
